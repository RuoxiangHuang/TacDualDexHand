...
class ShadowHandPickUpObjectEnvCfg(DirectRLEnvCfg):
    ...
    object = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(2.5, 0, 0.035)),
        spawn=sim_utils.UsdFileCfg(
            scale=(5, 5, 5),
...
class ShadowHandDualArmPickUpObjectScriptedEnvCfg(ShadowHandPickUpObjectEnvCfg):
    ...
    folder_joint_drive_stiffness: float = 100  # 驱动刚度（用于抵抗重力，0=无恢复力）
    folder_joint_drive_damping: float = 10  # 驱动阻尼（用于稳定运动，0=无恢复力）
...
class ShadowHandDualArmPickUpObjectScriptedEnv(ShadowHandPickUpObjectScriptedEnv):
    ...
    def __init__(self, cfg: ShadowHandDualArmPickUpObjectScriptedEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        folder_fixed_joint_prim_path = "/World/envs/env_0/folder/E_body_1/FixedJoint"
        folder_fixed_joint_prim = self.scene.stage.GetPrimAtPath(folder_fixed_joint_prim_path)
        if folder_fixed_joint_prim and folder_fixed_joint_prim.IsValid():
            folder_fixed_joint_prim.GetAttribute("physics:jointEnabled").Set(False)
            print(f"[DEBUG] 已禁用文件夹FixedJoint: {folder_fixed_joint_prim_path}")
        else:
            print(f"[WARNING] 无法找到文件夹FixedJoint: {folder_fixed_joint_prim_path}")
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 因为default_root_state可能被sim.reset()或folder.reset()覆盖为USD文件的默认值（可能是(0,0,0)）
        if hasattr(self, 'folder'):
            ...
            print(f"[INFO] Folder reset to position: {folder_pos.cpu().numpy()} (world: {root_state[0, :3].cpu().numpy()})")
            ...
            if hasattr(self.cfg, 'folder_joint_initial_angle') and self.cfg.folder_joint_initial_angle is not None:
                ...
                if hasattr(self.cfg, 'folder_joint_name') and self.cfg.folder_joint_name in joint_names:
                    joint_idx = joint_names.index(self.cfg.folder_joint_name)
                    folder_joint_pos[:, joint_idx] = self.cfg.folder_joint_initial_angle
            
            self.folder.write_joint_state_to_sim(folder_joint_pos, folder_joint_vel, env_ids=env_ids)
            
            # 调用reset以更新内部状态
            self.folder.reset(env_ids)
