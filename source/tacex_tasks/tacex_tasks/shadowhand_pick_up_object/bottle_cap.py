...
@configclass
class ShadowHandDualArmTwistBottleCapScriptedEnvCfg(ShadowHandDualArmPickUpObjectScriptedEnvCfg):
    """Config for dual-arm ShadowHand task using an articulation object (e.g., bottle cap mobility.usd).

    This is identical to the dual-arm pick-up scripted env config, except that the `object` is an Articulation.
    """

    # NOTE: the USD under this prim must be an articulation root (can contain multiple rigid bodies).
    object: ArticulationCfg = ArticulationCfg(
        prim_path="/World/envs/env_.*/object",
        init_state=ArticulationCfg.InitialStateCfg(pos=(0.5, 0.0, 0.035)),
        spawn=sim_utils.UsdFileCfg(
            usd_path=f"{TACEX_ASSETS_DATA_DIR}/Bottle/3517/mobility.usd",
            # Keep behavior close to the original rigid object defaults.
            rigid_props=RigidBodyPropertiesCfg(
                solver_position_iteration_count=16,
                solver_velocity_iteration_count=1,
                max_angular_velocity=1000.0,
                ...
                # Prevent the bottle from quickly sleeping when lightly contacted.
                sleep_threshold=0.0,
            ),
            articulation_props=sim_utils.ArticulationRootPropertiesCfg(
                ...
                solver_position_iteration_count=16,
                solver_velocity_iteration_count=1,
                # IMPORTANT: many mobility.usd assets ship with fixed root by default.
                # If fixed, the bottle will not move even when grasped.
                fix_root_link=False,
                sleep_threshold=0.0,
            ),
            # NOTE: scale will usually be overridden by YAML in the demo script.
            scale=(0.08, 0.08, 0.08),
        ),
        actuators={},  # passive object
    )


class ShadowHandDualArmTwistBottleCapScriptedEnv(ShadowHandDualArmPickUpObjectScriptedEnv):
    """Dual-arm ShadowHand scripted env where the 'object' is an Articulation (bottle/cap)."""

    cfg: ShadowHandDualArmTwistBottleCapScriptedEnvCfg

    def _setup_scene(self):
        """Setup scene with both robots and all sensors (object is an articulation)."""
        # Create left arm robot (cfg.robot was set to cfg.robot_left in __init__)
        self._robot = Articulation(self.cfg.robot)
        self.scene.articulations["robot"] = self._robot
        ...
        # Articulation object (can contain multiple rigid bodies)
        self.object = Articulation(self.cfg.object)
        self.scene.articulations["object"] = self.object

    def _reset_idx(self, env_ids: torch.Tensor | None):
        """Reset environments for dual-arm setup (articulation object)."""
        env_ids = env_ids if env_ids is not None else torch.arange(self.num_envs, device=self.device)

        # Reset base buffers (avoid parent implementation since it assumes RigidObject for `self.object`)
        DirectRLEnv._reset_idx(self, env_ids)

        # Reset articulation object root + joints
        try:
            ...
            # Best-effort: reset joints if present
            if hasattr(self.object.data, "default_joint_pos") and self.object.data.default_joint_pos is not None:
                joint_pos = self.object.data.default_joint_pos[env_ids]
                joint_vel = torch.zeros_like(joint_pos)
                self.object.write_joint_state_to_sim(joint_pos, joint_vel, env_ids=env_ids)
        except Exception as e:
            print(f"[WARNING] Failed to reset articulation object: {e}")

        # Update scene to get latest robot/object state (same as single-arm version)
        self.scene.update(dt=0.0)

        # Left arm - match dual-arm version's reset logic
        joint_pos_left = self._robot_left.data.default_joint_pos[env_ids]
        joint_vel_left = torch.zeros_like(joint_pos_left)
        self._robot_left.set_joint_position_target(joint_pos_left, env_ids=env_ids)
        self._robot_left.write_joint_state_to_sim(joint_pos_left, joint_vel_left, env_ids=env_ids)

        # Right arm - match dual-arm version's reset logic
        joint_pos_right = self._robot_right.data.default_joint_pos[env_ids].clone()
        if self._arm_joint_ids_right is not None and len(self._arm_joint_ids_right) >= 6:
            import math as _math

            arm_joint_targets = torch.tensor([_math.pi, -0.5, 1.0, -0.5, 0.0, 0.0], device=self.device)
            for i, joint_idx in enumerate(self._arm_joint_ids_right[:6]):
                if joint_idx < joint_pos_right.shape[1]:
                    joint_pos_right[:, joint_idx] = arm_joint_targets[i]
        ...
        # Reset finger targets for left arm
        if len(self._finger_joint_ids_left) > 0:
            if self.cfg.manual_finger_joint_positions_left is not None:
                manual_targets_left = self._get_manual_finger_targets_left()
                self.finger_targets_left[env_ids] = manual_targets_left[env_ids]
