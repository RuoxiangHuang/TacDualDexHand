"""ShadowHand dual-arm fixed grasp pose demo with different objects.

This script uses a fixed grasp pose (24 finger joints per arm) and allows testing
different objects with varying shapes and sizes. Based on single-arm version.

Usage:
    isaaclab -p ./scripts/demos/shadowhand_pick_up_object/run_shadowhand_dual_arm_gui_pose_new.py --num_envs 1 --object_name ball_wood
    isaaclab -p ./scripts/demos/shadowhand_pick_up_object/run_shadowhand_dual_arm_gui_pose_new.py --num_envs 1 --task twist_bottle_cap --config ./scripts/demos/shadowhand_pick_up_object/dual_arm_gui_pose_config.yaml
"""
...
parser = argparse.ArgumentParser(
    description="ShadowHand dual-arm fixed grasp pose demo with different objects"
)
parser.add_argument(
    "--config",
    type=str,
    default="./scripts/demos/shadowhand_pick_up_object/dual_arm_gui_pose_config.yaml",
    help="Path to YAML config for dual-arm demo (bases/End/sensors/cameras/joint poses).",
)
parser.add_argument(
    "--task",
    type=str,
    default="pick_up_object",
    choices=["pick_up_object", "twist_bottle_cap"],
    help=(
        "Which environment to run. "
        "'pick_up_object' uses a RigidObject for `object` (single rigid body). "
        "'twist_bottle_cap' uses an Articulation for `object` (e.g., Bottle/3517/mobility.usd)."
    ),
)
...
parser.add_argument(
    ...
    type=str,
    default="ball_wood",
    help="Object name (ball_wood, plate, or path to custom USD file)",
)
parser.add_argument(
    "--object_scale",
    type=float,
    nargs=3,
    default=[5.0, 5.0, 5.0],
    help="Object scale (x, y, z). Default: 5.0 5.0 5.0",
)
parser.add_argument(
    "--object_pos",
    type=float,
    nargs=3,
    default=[0.5, 0.0, 0.035],
    help="Object initial position (x, y, z). Default: 0.5 0.0 0.035",
)
...
from tacex_tasks.shadowhand_pick_up_object.bottle_cap import (
    ShadowHandDualArmPickUpObjectScriptedEnv,
    ShadowHandDualArmPickUpObjectScriptedEnvCfg,
    ShadowHandDualArmTwistBottleCapScriptedEnv,
    ShadowHandDualArmTwistBottleCapScriptedEnvCfg,
)
...
def _apply_object_joint_overrides_from_yaml(env, cfg_yaml: dict):
    """Apply joint drive/limit overrides to the articulation object (USD stage edits).

    This is intended for the `twist_bottle_cap` task where `env.object` is an Articulation and
    its USD may contain multiple rigid bodies under `/object`.

    YAML schema:
      object_joint_overrides:
        enabled: true
        joints:
          joint_name:
            type: revolute|prismatic   # optional (auto-detected if omitted)
            drive: {stiffness: float, damping: float}   # optional
            limits: {lower: float, upper: float}        # optional
    """
    over = _get_cfg(cfg_yaml, ["object_joint_overrides"], {}) or {}
    if not (isinstance(over, dict) and over.get("enabled", False)):
        return
    joints_cfg = over.get("joints", {}) if isinstance(over.get("joints", {}), dict) else {}
    if not isinstance(joints_cfg, dict) or len(joints_cfg) == 0:
        return

    try:
        stage = omni.usd.get_context().get_stage()
        if stage is None:
            print("[WARNING] USD stage not available; cannot apply object_joint_overrides.")
            return
    except Exception as e:
        print(f"[WARNING] Failed to get USD stage; cannot apply object_joint_overrides: {e}")
        return
    ...
    def _set_attr(prim, name: str, value):
        ...
        if not attr:
            try:
                # Use double for numeric properties (USD will accept float as well).
                attr = prim.CreateAttribute(name, type="double")
            except Exception:
                return False
        try:
            attr.Set(value)
    ...
    def _apply_to_joint(joint_prim, joint_name: str, spec: dict):
        if not isinstance(spec, dict):
            return

        # Ensure joint is enabled if field exists.
        ...
        joint_type_cfg = str(spec.get("type", "")).lower().strip()
        is_revolute = joint_prim.IsA(UsdPhysics.RevoluteJoint) or joint_type_cfg == "revolute"
        is_prismatic = joint_prim.IsA(UsdPhysics.PrismaticJoint) or joint_type_cfg == "prismatic"

        # Drive overrides
        drive = spec.get("drive", {}) if isinstance(spec.get("drive", {}), dict) else {}
        if isinstance(drive, dict) and (("stiffness" in drive) or ("damping" in drive)):
            stiffness = drive.get("stiffness", None)
            damping = drive.get("damping", None)
            if is_prismatic:
                if stiffness is not None:
                    _set_attr(joint_prim, "drive:linear:physics:stiffness", float(stiffness))
                if damping is not None:
                    _set_attr(joint_prim, "drive:linear:physics:damping", float(damping))
            elif is_revolute:
                # Try physics namespace first; also set non-physics fallback for compatibility
                if stiffness is not None:
                    _set_attr(joint_prim, "drive:angular:physics:stiffness", float(stiffness))
                    _set_attr(joint_prim, "drive:angular:stiffness", float(stiffness))
                if damping is not None:
                    _set_attr(joint_prim, "drive:angular:physics:damping", float(damping))
                    _set_attr(joint_prim, "drive:angular:damping", float(damping))

        # Limit overrides
        limits = spec.get("limits", {}) if isinstance(spec.get("limits", {}), dict) else {}
        if isinstance(limits, dict) and (("lower" in limits) or ("upper" in limits)):
            if "lower" in limits and limits["lower"] is not None:
                _set_attr(joint_prim, "physics:lowerLimit", float(limits["lower"]))
            if "upper" in limits and limits["upper"] is not None:
                _set_attr(joint_prim, "physics:upperLimit", float(limits["upper"]))

        print(f"[INFO] Applied object_joint_overrides for joint '{joint_name}'.")

    # Apply for each env instance
    for env_idx in range(int(getattr(env, "num_envs", 1))):
        obj_root = stage.GetPrimAtPath(f"/World/envs/env_{env_idx}/object")
        if not obj_root or not obj_root.IsValid():
            continue

        def _walk(prim):
            # If prim is a joint, match name and apply
            if prim.IsA(UsdPhysics.RevoluteJoint) or prim.IsA(UsdPhysics.PrismaticJoint):
                name = prim.GetName()
                if name in joints_cfg:
                    _apply_to_joint(prim, name, joints_cfg[name])
...
# Fixed grasp pose (24 joints) - extracted from debug output
# Joint order: WRJ2, WRJ1, FFJ4, LFJ5, MFJ4, RFJ4, THJ5, FFJ3, LFJ4, MFJ3, RFJ3, THJ4,
#              FFJ2, LFJ3, MFJ2, RFJ2, THJ3, FFJ1, LFJ2, MFJ1, RFJ1, THJ2, LFJ1, THJ1
FIXED_GRASP_POSE = {
    "WRJ2": -0.5236,   # -30.00°
    "WRJ1": -0.1981,   # -11.35°
    "FFJ4": -0.3491,   # -20.00°
...
def _apply_robot_joint_pos_overrides(env, arm_name: str, joint_pos_cfg: dict, arm_joint_preset: list[float] | None):
    ...
    # Start from default pose (stable baseline)
    joint_pos = robot.data.default_joint_pos.clone()

    # Apply 6-DoF arm preset if provided
    ...
    # Apply arbitrary joints from joint_pos_cfg (pattern match)
...
class CustomEnvWindow(BaseEnvWindow):
    ...
    def __init__(self, env: DirectRLEnv, window_name: str = "IsaacLab"):
        ...
        if ui_utils is not None:
            ...
            # Add object joint override frame (bottle/cap articulation difficulty)
            with self.ui_window_elements["main_vstack"]:
                self._build_object_joint_override_frame()
    ...
    def _build_object_joint_override_frame(self):
        """Build GUI controls for object_joint_overrides (drive stiffness/damping + joint limits)."""
        if ui_utils is None:
            return
        import omni.ui

        # Read current YAML (or fall back to defaults)
        cfg = getattr(self.env, "_cfg_yaml", {}) if hasattr(self.env, "_cfg_yaml") else {}
        over = _get_cfg(cfg, ["object_joint_overrides"], {}) or {}
        joints = over.get("joints", {}) if isinstance(over, dict) else {}
        if not isinstance(joints, dict):
            joints = {}

        def _joint_spec(name: str) -> dict:
            spec = joints.get(name, {}) if isinstance(joints.get(name, {}), dict) else {}
            drive = spec.get("drive", {}) if isinstance(spec.get("drive", {}), dict) else {}
            limits = spec.get("limits", {}) if isinstance(spec.get("limits", {}), dict) else {}
            return {
                "type": spec.get("type", ""),
                "stiffness": float(drive.get("stiffness", 0.0)) if drive.get("stiffness", None) is not None else 0.0,
                "damping": float(drive.get("damping", 0.0)) if drive.get("damping", None) is not None else 0.0,
                "lower": float(limits.get("lower", 0.0)) if limits.get("lower", None) is not None else 0.0,
                "upper": float(limits.get("upper", 0.0)) if limits.get("upper", None) is not None else 0.0,
            }

        j0 = _joint_spec("joint_0")
        j2 = _joint_spec("joint_2")

        enabled_default = bool(over.get("enabled", False)) if isinstance(over, dict) else False

        self.ui_window_elements["object_joint_override_frame"] = omni.ui.CollapsableFrame(
            title="Object Joint Overrides (Bottle/Cap Difficulty)",
            width=omni.ui.Fraction(1),
            height=0,
            collapsed=True,
            style=ui_utils.get_style(),
        ...
        with self.ui_window_elements["object_joint_override_frame"]:
            v = omni.ui.VStack(spacing=5, height=50)
            with v:
                # Toggle (avoid relying on ui_utils helpers; use omni.ui directly for compatibility)
                with omni.ui.HStack(spacing=8, height=0):
                    omni.ui.Label("Enable object_joint_overrides", width=220, tooltip="If disabled, Apply will do nothing.")
                    cb = omni.ui.CheckBox()
                    cb.model.set_value(bool(enabled_default))
                    self.ui_window_elements["obj_joint_over_enabled"] = cb.model

                # joint_0 (revolute)
                j0_frame = omni.ui.CollapsableFrame(
                    title="joint_0 (Revolute) - cap twist",
                    width=omni.ui.Fraction(1),
                    height=0,
                    collapsed=False,
                    style=ui_utils.get_style(),
                ...
                with j0_frame:
                    ...
                    with vv:
                        self.ui_window_elements["obj_j0_stiffness"] = ui_utils.combo_floatfield_slider_builder(
                            label="Drive stiffness",
                            default_val=float(j0["stiffness"]),
                            min=0.0,
                            max=5000.0,
                            step=1.0,
                            tooltip="drive:angular:*:stiffness (bigger => harder to twist).",
                        )[0]
                        self.ui_window_elements["obj_j0_damping"] = ui_utils.combo_floatfield_slider_builder(
                            label="Drive damping",
                            default_val=float(j0["damping"]),
                            min=0.0,
                            max=500.0,
                            step=0.1,
                            tooltip="drive:angular:*:damping (bigger => more resistance/stability).",
                        )[0]
                        self.ui_window_elements["obj_j0_lower"] = ui_utils.combo_floatfield_slider_builder(
                            label="Lower limit (rad)",
                            default_val=float(j0["lower"]),
                            min=-12.566,
                            max=0.0,
                            step=0.01,
                            tooltip="physics:lowerLimit for revolute joint (radians).",
                        )[0]
                        self.ui_window_elements["obj_j0_upper"] = ui_utils.combo_floatfield_slider_builder(
                            label="Upper limit (rad)",
                            default_val=float(j0["upper"]),
                            min=0.0,
                            max=12.566,
                            step=0.01,
                            tooltip="physics:upperLimit for revolute joint (radians).",
                        )[0]

                # joint_2 (prismatic)
                j2_frame = omni.ui.CollapsableFrame(
                    title="joint_2 (Prismatic) - cap lift",
                    width=omni.ui.Fraction(1),
                    height=0,
                    collapsed=True,
                    style=ui_utils.get_style(),
                ...
                with j2_frame:
                    vv = omni.ui.VStack(spacing=5, height=50)
                    with vv:
                        self.ui_window_elements["obj_j2_stiffness"] = ui_utils.combo_floatfield_slider_builder(
                            label="Drive stiffness",
                            default_val=float(j2["stiffness"]),
                            min=0.0,
                            max=5000.0,
                            step=1.0,
                            tooltip="drive:linear:*:stiffness (bigger => harder to translate).",
                        )[0]
                        self.ui_window_elements["obj_j2_damping"] = ui_utils.combo_floatfield_slider_builder(
                            label="Drive damping",
                            default_val=float(j2["damping"]),
                            min=0.0,
                            max=5000.0,
                            step=1.0,
                            tooltip="drive:linear:*:damping (bigger => more resistance/stability).",
                        )[0]
                        self.ui_window_elements["obj_j2_lower"] = ui_utils.combo_floatfield_slider_builder(
                            label="Lower limit (m)",
                            default_val=float(j2["lower"]),
                            min=-0.05,
                            max=0.0,
                            step=0.0005,
                            tooltip="physics:lowerLimit for prismatic joint (meters).",
                        )[0]
                        self.ui_window_elements["obj_j2_upper"] = ui_utils.combo_floatfield_slider_builder(
                            label="Upper limit (m)",
                            default_val=float(j2["upper"]),
                            min=0.0,
                            max=0.05,
                            step=0.0005,
                            tooltip="physics:upperLimit for prismatic joint (meters).",
                        )[0]

                self.ui_window_elements["apply_obj_joint_overrides_button"] = ui_utils.btn_builder(
                    type="button",
                    text="Apply Object Joint Overrides",
                    tooltip="Writes drive/limits into the USD stage for /World/envs/env_i/object joints.",
                    on_clicked_fn=self._apply_object_joint_overrides_from_gui,
                )

    def _apply_object_joint_overrides_from_gui(self):
        """Update env._cfg_yaml from GUI and apply to USD stage."""
        env = self.env
        try:
            if ui_utils is None:
                return
            enabled = bool(self.ui_window_elements["obj_joint_over_enabled"].get_value_as_bool())
            j0 = {
                "type": "revolute",
                "drive": {
                    "stiffness": float(self.ui_window_elements["obj_j0_stiffness"].get_value_as_float()),
                    "damping": float(self.ui_window_elements["obj_j0_damping"].get_value_as_float()),
                },
                "limits": {
                    "lower": float(self.ui_window_elements["obj_j0_lower"].get_value_as_float()),
                    "upper": float(self.ui_window_elements["obj_j0_upper"].get_value_as_float()),
                },
            }
            j2 = {
                "type": "prismatic",
                "drive": {
                    "stiffness": float(self.ui_window_elements["obj_j2_stiffness"].get_value_as_float()),
                    "damping": float(self.ui_window_elements["obj_j2_damping"].get_value_as_float()),
                },
                "limits": {
                    "lower": float(self.ui_window_elements["obj_j2_lower"].get_value_as_float()),
                    "upper": float(self.ui_window_elements["obj_j2_upper"].get_value_as_float()),
                },
            }

            if not hasattr(env, "_cfg_yaml") or not isinstance(env._cfg_yaml, dict):
                env._cfg_yaml = {}
            env._cfg_yaml["object_joint_overrides"] = {"enabled": enabled, "joints": {"joint_0": j0, "joint_2": j2}}

            if enabled:
                _apply_object_joint_overrides_from_yaml(env, env._cfg_yaml)
                print("[INFO] Applied object_joint_overrides from GUI.")
            else:
                print("[INFO] object_joint_overrides disabled in GUI; no USD edits applied.")
        except Exception as e:
            import traceback as _tb
            print(f"[WARNING] Failed to apply object_joint_overrides from GUI: {e}")
            _tb.print_exc()
    ...
    def _save_yaml_snapshot(self):
        ...
        try:
            ...
            # Preserve object_joint_overrides (for bottle-cap articulation difficulty)
            object_joint_overrides = cfg_in.get("object_joint_overrides", {}) if isinstance(cfg_in, dict) else {}
            ...
            out_cfg = {
                ...
                "assets": assets,
                "object_joint_overrides": object_joint_overrides,
    ...
    def _build_finger_joint_control_frame(self, arm_name: str):
        ...
        with self.ui_window_elements[f"finger_joint_frame_{arm_name}"]:
            ...
            with self.ui_window_elements[f"finger_joint_vstack_{arm_name}"]:
                ...
                # Add "Apply current as Close" button (updates the close preset used by Reset-to-Grasp)
                self.ui_window_elements[f"apply_as_close_button_{arm_name}"] = ui_utils.btn_builder(
                    type="button",
                    text=f"Apply Current as Close Pose ({arm_name.upper()})",
                    tooltip=(
                        f"Saves the current finger joint targets as the '{arm_name}' close pose. "
                        "Next time you click 'Reset to Fixed Grasp Pose', it will use this saved pose."
                    ),
                    on_clicked_fn=lambda: self._apply_current_as_close_pose(arm_name),
                )
    ...
    def _apply_current_as_close_pose(self, arm_name: str):
        """Save current finger targets as the 'closed' pose for this arm (runtime + YAML)."""
        env = self.env
        try:
            if arm_name == "left":
                robot = env._robot_left
                finger_joint_ids = env._finger_joint_ids_left if hasattr(env, "_finger_joint_ids_left") else []
                finger_targets = env.finger_targets_left if hasattr(env, "finger_targets_left") else None
            else:
                robot = env._robot_right
                finger_joint_ids = env._finger_joint_ids_right if hasattr(env, "_finger_joint_ids_right") else []
                finger_targets = env.finger_targets_right if hasattr(env, "finger_targets_right") else None

            if finger_targets is None or len(finger_joint_ids) == 0:
                print(f"[WARNING] Finger targets not available; cannot apply-as-close for {arm_name}.")
                return

            # Build a compact dict keyed by FIXED_GRASP_POSE patterns (WRJ2, FFJ4, ...)
            # This keeps the YAML format consistent with existing snapshots.
            joint_names = list(robot.joint_names)
            out: dict[str, float] = {}
            for pat in FIXED_GRASP_POSE.keys():
                v = None
                for i, j_idx in enumerate(finger_joint_ids):
                    jn = joint_names[int(j_idx)]
                    if str(pat).upper() in jn.upper() or jn.upper() in str(pat).upper():
                        v = float(finger_targets[0, i].item())
                        break
                if v is not None:
                    out[str(pat)] = float(v)

            if arm_name == "left":
                env._finger_pose_closed_left = out
                if hasattr(env, "cfg") and hasattr(env.cfg, "manual_finger_joint_positions_left"):
                    env.cfg.manual_finger_joint_positions_left = out
            else:
                env._finger_pose_closed_right = out
                if hasattr(env, "cfg") and hasattr(env.cfg, "manual_finger_joint_positions_right"):
                    env.cfg.manual_finger_joint_positions_right = out

            # Update YAML (so snapshot captures it)
            if not hasattr(env, "_cfg_yaml") or not isinstance(env._cfg_yaml, dict):
                env._cfg_yaml = {}
            fp = env._cfg_yaml.get("finger_poses", {})
            if not isinstance(fp, dict):
                fp = {}
            closed = fp.get("closed", {})
            if not isinstance(closed, dict):
                closed = {}
            per_arm = closed.get("per_arm", {})
            if not isinstance(per_arm, dict):
                per_arm = {}
            per_arm.setdefault("left", {})
            per_arm.setdefault("right", {})
            per_arm[arm_name] = out
            closed["per_arm"] = per_arm
            # Keep base joint_pos (if present) as-is; if missing, default to FIXED_GRASP_POSE for readability.
            if "joint_pos" not in closed or not isinstance(closed.get("joint_pos", {}), dict):
                closed["joint_pos"] = FIXED_GRASP_POSE
            fp["closed"] = closed
            if "initial" not in fp:
                fp["initial"] = "closed"
            env._cfg_yaml["finger_poses"] = fp

            print(f"[INFO] Saved current finger targets as CLOSE pose ({arm_name} arm).")
        except Exception as e:
            import traceback as _tb
            print(f"[WARNING] Failed to apply current as close pose ({arm_name} arm): {e}")
            _tb.print_exc()


def get_object_usd_path(object_name: str) -> str:
    """Get USD file path for the object.
    
    Args:
        object_name: Name of the object or path to USD file
    ...
    if object_name.endswith('.usd'):
        return object_name
    
    # Map object names to USD paths
    object_map = {
        "ball_wood": f"{TACEX_ASSETS_DATA_DIR}/Props/ball_wood.usd",
        "plate": f"{TACEX_ASSETS_DATA_DIR}/Props/plate.usd",
    ...
    if object_name in object_map:
        ...
        # Try to construct path from object name
        return f"{TACEX_ASSETS_DATA_DIR}/Props/{object_name}.usd"


def run_simulator(env: ShadowHandDualArmPickUpObjectScriptedEnv):
    """Runs the simulation loop with fixed grasp pose."""
    print(f"Starting ShadowHand dual-arm fixed grasp pose simulation with {env.num_envs} envs")
    print(f"Object: {args_cli.object_name}")
    print(f"Object scale: {args_cli.object_scale}")
    print(f"Object position: {args_cli.object_pos}")
    print(f"\nFixed grasp pose (24 joints per arm):")
    for joint_name, value in FIXED_GRASP_POSE.items():
        print(f"  {joint_name}: {value:.4f} rad ({np.degrees(value):.2f}°)")
    
    try:
        env.reset()

        # Attach YAML to env so the rest of the script (End init, sensors, GUI defaults) can read it.
        if not hasattr(env, "_cfg_yaml"):
            env._cfg_yaml = CFG

        # Attach finger pose presets from YAML (used by GUI reset buttons).
        fp = _get_cfg(getattr(env, "_cfg_yaml", {}), ["finger_poses"], {}) or {}
        closed = fp.get("closed", {}) if isinstance(fp, dict) else {}
        open_pose = fp.get("open", {}) if isinstance(fp, dict) else {}
        ...
        env._finger_pose_closed_left = {**(closed_base if isinstance(closed_base, dict) else {}), **(closed_per.get("left", {}) if isinstance(closed_per, dict) else {})}
        env._finger_pose_closed_right = {**(closed_base if isinstance(closed_base, dict) else {}), **(closed_per.get("right", {}) if isinstance(closed_per, dict) else {})}

        open_mode = open_pose.get("mode", "lower_limits") if isinstance(open_pose, dict) else "lower_limits"
        open_base = (open_pose.get("joint_pos", {}) if isinstance(open_pose, dict) else {}) or {}
        open_per = (open_pose.get("per_arm", {}) if isinstance(open_pose, dict) else {}) or {}
        env._finger_pose_open_left = {
            "mode": open_mode,
            "joint_pos": {**(open_base if isinstance(open_base, dict) else {}), **(open_per.get("left", {}) if isinstance(open_per, dict) else {})},
        }
        env._finger_pose_open_right = {
            "mode": open_mode,
            "joint_pos": {**(open_base if isinstance(open_base, dict) else {}), **(open_per.get("right", {}) if isinstance(open_per, dict) else {})},
        }

        # Apply YAML initial joint poses (all joints + optional 6-DoF arm preset) once at startup.
        try:
        ...
        # Initialize finger joint controls in GUI (after env is ready)
        if env._window is not None and hasattr(env._window, '_build_finger_joint_control_frame'):
            env._window._build_finger_joint_control_frame("left")
            env._window._build_finger_joint_control_frame("right")
        
        # List of all sensors for visualization (10 sensors: 5 per arm)
        finger_sensors = [
            ("ff_left", env.gelsighthand_ff_left, env.cfg.gelsighthand_ff_left),
            ("mf_left", env.gelsighthand_mf_left, env.cfg.gelsighthand_mf_left),
            ("rf_left", env.gelsighthand_rf_left, env.cfg.gelsighthand_rf_left),
            ("lf_left", env.gelsighthand_lf_left, env.cfg.gelsighthand_lf_left),
            ("th_left", env.gelsighthand_th_left, env.cfg.gelsighthand_th_left),
            ("ff_right", env.gelsighthand_ff_right, env.cfg.gelsighthand_ff_right),
            ("mf_right", env.gelsighthand_mf_right, env.cfg.gelsighthand_mf_right),
            ("rf_right", env.gelsighthand_rf_right, env.cfg.gelsighthand_rf_right),
            ("lf_right", env.gelsighthand_lf_right, env.cfg.gelsighthand_lf_right),
            ("th_right", env.gelsighthand_th_right, env.cfg.gelsighthand_th_right),
        ]
        
        # Enable debug visualization for tactile sensors
        for sensor_name, sensor, sensor_cfg in finger_sensors:
            if sensor_cfg.debug_vis:
                for data_type in sensor_cfg.data_types:
                    if hasattr(sensor, '_prim_view') and sensor._prim_view is not None:
                        if len(sensor._prim_view.prims) > 0:
                            try:
                                sensor._prim_view.prims[0].GetAttribute(f"debug_{data_type}").Set(True)
                                print(f"[INFO] Enabled debug visualization for {sensor_name} sensor: {data_type}")
                            except Exception as e:
                                print(f"[WARNING] Failed to enable debug visualization for {sensor_name} sensor ({data_type}): {e}")
        ...
        initial_end_pos_left = np.array(end_left_cfg.get("pos_w", [-1.0, 0.0, 0.3]), dtype=np.float32)
        initial_end_quat_left = np.array(end_left_cfg.get("quat_wxyz", [0.70711, 0.70711, 0.0, 0.0]), dtype=np.float32)
        initial_end_pos_right = np.array(end_right_cfg.get("pos_w", [2.0, 0.0, 0.3]), dtype=np.float32)
        ...
        # Apply per-sensor camera local pose from YAML (USD edit). Supports num_envs==1.
        try:
            stage = omni.usd.get_context().get_stage()
            defaults = _get_cfg(getattr(env, "_cfg_yaml", {}), ["tactile_sensors", "defaults"], {}) or {}
            per_sensors = _get_cfg(getattr(env, "_cfg_yaml", {}), ["tactile_sensors", "sensors"], {}) or {}
            default_pose = (defaults.get("camera_pose_local", {}) or {}) if isinstance(defaults, dict) else {}
            for sensor_key, _sensor, sensor_cfg in finger_sensors:
                per = per_sensors.get(sensor_key, {}) if isinstance(per_sensors, dict) else {}
                per_pose = (per.get("camera_pose_local", {}) or {}) if isinstance(per, dict) else {}
                pose_cfg = {}
                if isinstance(default_pose, dict):
                    pose_cfg.update(default_pose)
                if isinstance(per_pose, dict):
                    pose_cfg.update(per_pose)
                if not isinstance(pose_cfg, dict) or not pose_cfg.get("enabled", False):
                    continue
                pos = _as_tuple3(pose_cfg.get("pos"), (0.0, 0.0, 0.0))
                quat = _as_tuple4_wxyz(pose_cfg.get("quat_wxyz"), (1.0, 0.0, 0.0, 0.0))
                cam_expr = f"{sensor_cfg.prim_path}/Camera"
                if "env_.*/" in cam_expr:
                    if env.num_envs == 1:
                        cam_path = cam_expr.replace("env_.*/", "env_0/")
                        _apply_camera_local_pose(stage, cam_path, pos, quat)
                    else:
                        print(f"[WARNING] camera_pose_local for {sensor_key} requires num_envs==1. Skipping.")
                else:
                    _apply_camera_local_pose(stage, cam_expr, pos, quat)
        except Exception as e:
            print(f"[WARNING] Failed to apply sensor camera local pose overrides: {e}")
        ...
        while simulation_app.is_running() and not should_exit:
            if env._window is not None and env._window.reset:
                ...
                # Apply YAML initial joint poses (all joints + optional arm preset)
                try:
                    left_joint_cfg = _get_cfg(getattr(env, "_cfg_yaml", {}), ["robot_bases", "left", "joint_pos"], {}) or {}
                    right_joint_cfg = _get_cfg(getattr(env, "_cfg_yaml", {}), ["robot_bases", "right", "joint_pos"], {}) or {}
                    left_arm_preset = None
                ...
                # Reset fingers to fixed grasp pose
                # Update GUI controls to reflect current finger_targets
                if hasattr(env._window, 'finger_joint_controls_left'):
                    for joint_idx, control_info in env._window.finger_joint_controls_left.items():
                        if joint_idx < env.finger_targets_left.shape[1]:
                            try:
                                control_info['control'].set_value(env.finger_targets_left[0, joint_idx].item())
                            except Exception as e:
                                pass
                
                if hasattr(env._window, 'finger_joint_controls_right'):
                    for joint_idx, control_info in env._window.finger_joint_controls_right.items():
                        if joint_idx < env.finger_targets_right.shape[1]:
                            try:
                                control_info['control'].set_value(env.finger_targets_right[0, joint_idx].item())
                            except Exception as e:
                                pass
                
                # Reset End positions and rotations to initial values
                env.scene.update(dt=0.0)
                initial_end_pos_tensor_left = torch.tensor(initial_end_pos_left, device=env.device, dtype=torch.float32).unsqueeze(0)
                initial_end_quat_tensor_left = torch.tensor(initial_end_quat_left, device=env.device, dtype=torch.float32).unsqueeze(0)
                env.end_prim_view_left.set_world_poses(
                ...
                # Update scene to ensure End object positions and robot states are synchronized
                env.scene.update(dt=0.0)
                
                # Update ik_commands (using updated robot state)
                # Left arm
                positions, orientations = env.end_prim_view_left.get_world_poses()
                root_pos_w = env._robot_left.data.root_link_pos_w
                root_quat_w = env._robot_left.data.root_link_quat_w
            ...
            # Update isaac buffers (also updates sensors) - MUST be before reading robot state
            env.scene.update(dt=env.physics_dt)
            
            # Read End virtual object positions and update ik_commands (using updated robot state)
            # Left arm
            positions, orientations = env.end_prim_view_left.get_world_poses()
            root_pos_w = env._robot_left.data.root_link_pos_w
            root_quat_w = env._robot_left.data.root_link_quat_w
            ...
            # Render scene for cameras (used by sensors)
            env.sim.render()
            ...
            if step_count % 1000 == 0:
                ...
                # Print object height
                object_pos_w = env.object.data.root_pos_w
                avg_object_height = object_pos_w[:, 2].mean().item()
                print(f"  Average object height: {avg_object_height:.4f}")
    finally:
        pass
...
def main():
    """Main function."""
    # -------------------------------------------------------------------------
    # Select task / env (object type differs: RigidObject vs Articulation)
    # -------------------------------------------------------------------------
    task = str(getattr(args_cli, "task", "pick_up_object")).lower()
    obj_usd_from_yaml = _get_cfg(CFG, ["assets", "object", "usd_path"], None)
    # If user points object to a mobility articulation, auto-switch to the articulation env to avoid runtime error.
    if task == "pick_up_object" and isinstance(obj_usd_from_yaml, str) and "mobility.usd" in obj_usd_from_yaml:
        print(
            "[INFO] Detected 'mobility.usd' in YAML assets.object.usd_path. "
            "Switching task to 'twist_bottle_cap' (Articulation object) to avoid RigidObject multi-body error."
        )
        task = "twist_bottle_cap"

    # Define simulation env
    if task == "twist_bottle_cap":
        env_cfg = ShadowHandDualArmTwistBottleCapScriptedEnvCfg()
    else:
    ...
    # -------------------------------------------------------------------------
    # Apply YAML robot base poses (positions in meters; quat is wxyz in YAML but
    # Articulation init_state rot uses xyzw)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # End-effector task frame offset (make EE-to-End offset = 0 by default)
    # This sets cfg.body_offset_pos/body_offset_rot used in frame pose and jacobian.
    # -------------------------------------------------------------------------
    ...
    # Left offset goes into env_cfg for initialization (dual env uses this for left arm).
    env_cfg.body_offset_pos = left_pos
    env_cfg.body_offset_rot = left_quat

    # -------------------------------------------------------------------------
    # Apply YAML tactile sensor defaults + per-sensor overrides to all 10 sensors
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Apply YAML environment RGB ArmCamera settings
    # -------------------------------------------------------------------------
    cam = _get_cfg(CFG, ["env_camera", "arm_camera"], {}) or {}
    if isinstance(cam, dict) and hasattr(env_cfg, "arm_camera"):
        pos = _get_cfg(CFG, ["env_camera", "arm_camera", "pos"], None)
        quat = _get_cfg(CFG, ["env_camera", "arm_camera", "quat_wxyz"], None)
        res = _get_cfg(CFG, ["env_camera", "arm_camera", "resolution"], None)
        clip = _get_cfg(CFG, ["env_camera", "arm_camera", "clipping_range"], None)
        if pos is not None:
            env_cfg.arm_camera.offset.pos = _as_tuple3(pos, env_cfg.arm_camera.offset.pos)
        if quat is not None:
            env_cfg.arm_camera.offset.rot = _as_tuple4_wxyz(quat, env_cfg.arm_camera.offset.rot)
        if res is not None and len(res) == 2:
            env_cfg.arm_camera.width = int(res[0])
            env_cfg.arm_camera.height = int(res[1])
        if clip is not None and hasattr(env_cfg.arm_camera, "spawn") and hasattr(env_cfg.arm_camera.spawn, "clipping_range"):
            env_cfg.arm_camera.spawn.clipping_range = (float(clip[0]), float(clip[1]))

    # -------------------------------------------------------------------------
    # Apply YAML IK controller overrides (best-effort, field names depend on version)
    # -------------------------------------------------------------------------
    ik_over = _get_cfg(CFG, ["ik_controller", "overrides"], {}) or {}
    if isinstance(ik_over, dict):
        for k, v in ik_over.items():
            if hasattr(env_cfg.ik_controller_cfg_left, k):
                setattr(env_cfg.ik_controller_cfg_left, k, v)
            if hasattr(env_cfg.ik_controller_cfg_right, k):
                setattr(env_cfg.ik_controller_cfg_right, k, v)

    # -------------------------------------------------------------------------
    # Apply YAML USD asset overrides (ground_plate + object)
    # -------------------------------------------------------------------------
    assets = _get_cfg(CFG, ["assets"], {}) or {}
    if isinstance(assets, dict):
        # ground_plate
        gp = assets.get("ground_plate", {}) if isinstance(assets.get("ground_plate", {}), dict) else {}
        gp_usd = gp.get("usd_path", None)
        gp_pos = gp.get("pos", None)
        gp_scale = gp.get("scale", None)
        if gp_usd is not None:
            env_cfg.plate.spawn.usd_path = _expand_env_vars(str(gp_usd))
        if gp_pos is not None:
            env_cfg.plate.init_state.pos = _as_tuple3(gp_pos, env_cfg.plate.init_state.pos)
        # UsdFileCfg may or may not have scale attribute depending on version/usage
        if gp_scale is not None and hasattr(env_cfg.plate.spawn, "scale"):
            env_cfg.plate.spawn.scale = _as_tuple3(gp_scale, getattr(env_cfg.plate.spawn, "scale", (1.0, 1.0, 1.0)))

        # object
        obj = assets.get("object", {}) if isinstance(assets.get("object", {}), dict) else {}
        obj_usd = obj.get("usd_path", None)
        obj_pos = obj.get("pos", None)
        obj_scale = obj.get("scale", None)
        if obj_usd is not None:
            env_cfg.object.spawn.usd_path = _expand_env_vars(str(obj_usd))
        if obj_pos is not None:
            env_cfg.object.init_state.pos = _as_tuple3(obj_pos, env_cfg.object.init_state.pos)
        if obj_scale is not None and hasattr(env_cfg.object.spawn, "scale"):
            env_cfg.object.spawn.scale = _as_tuple3(obj_scale, getattr(env_cfg.object.spawn, "scale", (1.0, 1.0, 1.0)))
    
    # Enable debug visualization for sensors
    env_cfg.gelsighthand_ff_left.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_mf_left.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_rf_left.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_lf_left.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_th_left.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_ff_right.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_mf_right.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_rf_right.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_lf_right.debug_vis = args_cli.debug_vis
    env_cfg.gelsighthand_th_right.debug_vis = args_cli.debug_vis
    
    # Configure object (CLI still supported; if YAML assets.object.usd_path is provided, it wins)
    object_usd_path = env_cfg.object.spawn.usd_path if getattr(env_cfg.object.spawn, "usd_path", None) else get_object_usd_path(args_cli.object_name)
    object_scale = tuple(getattr(env_cfg.object.spawn, "scale", tuple(args_cli.object_scale)))
    object_pos = tuple(env_cfg.object.init_state.pos) if getattr(env_cfg.object, "init_state", None) else tuple(args_cli.object_pos)
    
    print(f"[INFO] Using object: {object_usd_path}")
    print(f"[INFO] Object scale: {object_scale}")
    print(f"[INFO] Object position: {object_pos}")
    
    # Update object configuration (keep env_cfg consistent with what we print)
    env_cfg.object.spawn.usd_path = object_usd_path
    if hasattr(env_cfg.object.spawn, "scale"):
        env_cfg.object.spawn.scale = object_scale
    env_cfg.object.init_state.pos = object_pos
    
    # -------------------------------------------------------------------------
    # Apply YAML finger open/closed presets and choose initial pose
    # -------------------------------------------------------------------------
    ...
    open_base = (open_pose.get("joint_pos", {}) if isinstance(open_pose, dict) else {}) or {}
    open_per = (open_pose.get("per_arm", {}) if isinstance(open_pose, dict) else {}) or {}
    left_open = {**(open_base if isinstance(open_base, dict) else {}), **(open_per.get("left", {}) if isinstance(open_per, dict) else {})}
    right_open = {**(open_base if isinstance(open_base, dict) else {}), **(open_per.get("right", {}) if isinstance(open_per, dict) else {})}

    if str(fp_initial).lower() == "open":
        if open_mode == "explicit":
            env_cfg.manual_finger_joint_positions_left = left_open if len(left_open) > 0 else None
            env_cfg.manual_finger_joint_positions_right = right_open if len(right_open) > 0 else None
        else:
    ...
    # Create environment
    if task == "twist_bottle_cap":
        env = ShadowHandDualArmTwistBottleCapScriptedEnv(cfg=env_cfg)
        window_title = "ShadowHand Dual-Arm Twist Bottle Cap (GUI Pose)"
    else:
        env = ShadowHandDualArmPickUpObjectScriptedEnv(cfg=env_cfg)
        window_title = "ShadowHand Dual-Arm Fixed Grasp Pose"

    # IMPORTANT: In GUI/end-effector teleop mode we must not auto-reset.
    # Otherwise, if the robot base is moved (e.g., z=0.5 in YAML), the done condition
    # based on object height in base frame can trigger every step and continuously overwrite IK/joints.
    env._disable_auto_reset = True

    # Attach YAML and finger pose presets to env (used by run_simulator + GUI)
    env._cfg_yaml = CFG
    env._finger_pose_closed_left = left_closed
    env._finger_pose_closed_right = right_closed
    env._finger_pose_open_left = {"mode": open_mode, "joint_pos": left_open}
    env._finger_pose_open_right = {"mode": open_mode, "joint_pos": right_open}

    # Apply right-arm EE offset after env is created (dual env stores separate tensors for right arm).
    try:
    ...
    # Disable automatic finger control to allow GUI control
    env._auto_finger_control_left = False
    env._auto_finger_control_right = False

    # Apply articulation joint overrides (only meaningful for twist_bottle_cap task).
    if task == "twist_bottle_cap":
        _apply_object_joint_overrides_from_yaml(env, CFG)
    
    # Create custom window
    env._window = CustomEnvWindow(env, window_name=window_title)
